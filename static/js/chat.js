...
const MODELS = {
  'nexa-pro': { name: 'Nexa Pro', icon: '<svg class="icon-svg-sm"><use href="#icon-brain"/></svg>', model: 'Meta-Llama-3.1-405B-Instruct' },
  'nexa-flash': { name: 'Nexa Flash', icon: '<svg class="icon-svg-sm"><use href="#icon-flash"/></svg>', model: 'Meta-Llama-3.1-8B-Instruct' },
  'nexa-vision': { name: 'Nexa Vision', icon: '<svg class="icon-svg-sm"><use href="#icon-vision"/></svg>', model: 'Qwen2-VL-72B-Instruct' },
  'nexa-code': { name: 'Nexa Code', icon: '<svg class="icon-svg-sm"><use href="#icon-code"/></svg>', model: 'Qwen2.5-Coder-32B-Instruct' },
  'nexa-research': { name: 'Nexa Research', icon: '<svg class="icon-svg-sm"><use href="#icon-research"/></svg>', model: 'QwQ-32B-Preview' }
};
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.addEventListener('click', (e) => {
    // Image click handler - open full image in new tab
    if (e.target.classList.contains('chat-image') || (e.target.tagName === 'IMG' && e.target.closest('.message-content'))) {
      e.preventDefault();
...
window.checkAuth = async function () {
  ...
  if (d.logged_in) {
    ...
    if (d.photo) {
      [sidebarAvatar, headerAvatar, diaryAvatar].forEach(av => {
        if (av) {
...
window.updateModelUI = function () {
  ...
  document.getElementById('model-badge-icon').innerHTML = m.icon;
...
const MODES = {
  chat: { name: 'Chat', icon: '<svg class="icon-svg-sm"><use href="#icon-chat"/></svg>' },
  diary: { name: 'Diary', icon: '<svg class="icon-svg-sm"><use href="#icon-diary"/></svg>' }
};
...
window.selectMode = function (el) {
  ...
  // Validate mode
  if (!MODES[currentMode]) currentMode = 'chat';
...
window.updateModeUI = function () {
  ...
  document.getElementById('mode-badge-icon').innerHTML = m.icon;
...
window.chatHTML = function (chat) {
  ...
  const savedDot = chat.is_saved ? '<span class="chat-item-saved"><svg class="icon-svg-sm"><use href="#icon-save"/></svg></span>' : '';
  const pinDot = chat.is_pinned ? '<span class="chat-item-saved"><svg class="icon-svg-sm"><use href="#icon-pin"/></svg></span>' : '';
...
window.openChat = async function (chatId, isNew = false) {
  // Cancel any ongoing message sending
  if (state.isSending) {
...
window.sendMessage = async function () {
  ...
  const requestChatId = state.currentChatId; // Store chat ID at request time

  try {
    const data = await fetchJSON(`/api/chats/${requestChatId}/messages`, 'POST', { content, web_search: state.webSearchEnabled });
    
    // Check if user switched to different chat while waiting for response
    if (state.currentChatId !== requestChatId) {
      return; // Ignore response if chat was switched
    }
    ...
    const idx = state.allChats.findIndex(c => c.id === requestChatId);
    if (idx !== -1) { state.allChats[idx].title = data.chat_title; }
    else { await loadChats(); }
    renderChats(document.getElementById('chat-search').value);
    const chatEl = document.getElementById(`chat-item-${requestChatId}`);
...
window.sendMessageWithFile = async function (content) {
  ...
  try {
    const response = await fetch(`/api/chats/${requestChatId}/messages`, {
      method: 'POST',
      headers: {
...
window.updateDiaryStats = function () {
  const totalEl = document.getElementById('diary-total-entries');
  const streakEl = document.getElementById('diary-streak');
  const lastEl = document.getElementById('diary-last-updated');
  
  if (totalEl) totalEl.textContent = diaryEntries.length;
  
  // Calculate streak (consecutive days with entries)
  let streak = 0;
  if (diaryEntries.length > 0) {
    const sorted = [...diaryEntries].sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  ...
  // Last updated
  if (lastEl && diaryEntries.length > 0) {
    const sorted = [...diaryEntries].sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
    ...
    if (diffDays === 0) lastEl.textContent = 'Today';
    else if (diffDays === 1) lastEl.textContent = 'Yesterday';
    else if (diffDays > 1) lastEl.textContent = `${diffDays}d ago`;
    else lastEl.textContent = 'Today';
  }
  
  // Sidebar stats
  const sidebarTotal = document.getElementById('sidebar-total-entries');
  const sidebarWeek = document.getElementById('sidebar-week-entries');
  const sidebarStreak = document.getElementById('sidebar-streak');
  
  if (sidebarTotal) sidebarTotal.textContent = diaryEntries.length;
  
  // This week entries
  if (sidebarWeek) {
    const weekAgo = Date.now() - (7 * 86400000);
    const thisWeek = diaryEntries.filter(e => new Date(e.created_at) >= weekAgo).length;
    sidebarWeek.textContent = thisWeek;
  }
  
  if (sidebarStreak) {
    sidebarStreak.textContent = streak + (streak === 1 ? ' day' : ' days');
  }
}

window.renderDiaryEntries = function (filter = '') {
  const list = document.getElementById('diary-list');
  const placeholders = document.getElementById('diary-placeholders');
  if (!list) return;
  ...
  if (!filtered.length) {
    ...
    if (placeholders) placeholders.style.display = 'block';
    return;
  }
...
window.showFilePreview = function (file, isImage) {
  ...
  if (isImage) {
    ...
    reader.onload = (e) => {
      ...
      img.onclick = () => viewFile(dataUrl, file.name, true);
      
      const info = document.createElement('div');
      info.className = 'file-preview-info';
      info.innerHTML = `
        <div class="file-preview-name" style="cursor:pointer">${file.name}</div>
        <div class="file-preview-size">${size}</div>
      `;
      info.querySelector('.file-preview-name').onclick = () => viewFile(dataUrl, file.name, true);
...
window.viewFile = function (dataUrl, fileName, isImage) {
  if (isImage) {
    ...
    if (win) {
      ...
      showToast('Please allow popups to view image', 'error');
    }
  }
}
